class MenusController < ApplicationController
  ...
  def index
    @menus = current_user.menus.includes(recipes: [ cuisine_flag_attachment: :blob ]).order(date: :desc)
  ...
  def show
    @available_recipes = current_user.recipes.with_attached_cuisine_flag.where.not(id: @menu.recipe_ids)
  end

  def new
    @menu = current_user.menus.build
    @recipes = current_user.recipes.with_attached_cuisine_flag
    @occasions = Menu::OCCASIONS
    @cuisines = Recipe::CUISINES
  ...
  def set_menu
    @menu = current_user.menus.includes(recipes: [ cuisine_flag_attachment: :blob ]).find(params[:id])
  end
